...
router.post('', (req, res) => {
  ...
  var rendezVous = new RendezVous({
    ...
    sortedDate: req.body.dateDeRendezVous,
      dateFinDeRendezVous: req.body.dateFinDeRendezVous,
...
//(new Date(CurDate)).format("llll")
//moment().format('llll')
//var CDate = new Date(CurDate);
 //.sort({'dateDeRendezVous' : 1})
 //
 //var c = Date.parse(RendezVous.dateDeRendezVous);
router.get("", (req, res, next) => {
 
 var cc = new Date('dateDeRendezVous')
 console.log(RendezVous.dateDeRendezVous)
  RendezVous.find().sort({'sortedDate': -1}).then(documents => {
...
router.get("/rendi/:idTalent", (req, res, next) => {
  RendezVous.find({ idTalent: req.params.idTalent }).sort({'sortedDate': 1}).then(document => {
    res.status(200).json({
      message: " RendezVous fetched successfully!",
      RendezVous: document
    });
