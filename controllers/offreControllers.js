...
//postuler un offre pour un client
// body{
// 	"tab" : ["http://localhost:3000/uploads/document-uc-gestion-des-offres.jpg", "http://localhost:3000/uploads/cheque-uc-gestion-des-offres.jpg"] ,
// 	"emails" :[ "salimaze.karoui@gmail.com " , "aymenaze.karoui@gmail.com" ],
// 	"idTalent": "598",
// 	"idOffre": "7894"
// }
...
router.post('/offre/send/att',	async function(req, res, next) {
   ...
   var attachementList = [];
    ...
    var pos = TableausURLS[k].indexOf('uploads/')+8;

      var nom = TableausURLS[k].substring(pos,TableausURLS[k].length);
      console.log("nom"+nom);
...
router.post('/', (req, res) => {
    ...
    off.save((err, doc) => {
        if (!err) { res.send(doc); }
        else {console.log(err) ;res.send('Error in Offre Save :' + JSON.stringify(err, undefined, 2)); }
    });
});
